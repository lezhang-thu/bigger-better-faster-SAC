...
def select_action(
    ...
    logits, samples = two_results(logits_w_samples, state, key)
...
def two_results(func, states, rngs):
    return func(states, rngs)


@functools.partial(jax.vmap, in_axes=(None, 0, 0, None), axis_name="batch")
def get_logits(model, states, actions, do_rollout):
    results = model(
        states,
        actions=actions,
        do_rollout=do_rollout,
...
def get_q_values(model, states, actions, do_rollout):
    results = model(
        states,
        actions=actions,
        do_rollout=do_rollout,
...
@functools.partial(jax.vmap, in_axes=(None, 0), axis_name="batch")
@functools.partial(jax.vmap, in_axes=(None, 0), axis_name="time")
def get_spr_targets(model, states):
...
def train(
    ...
    def train_one_batch(state, inputs):
        ...
        def policy_loss(params, states, key):
            ...
            return -(
                (jax.lax.stop_gradient(q_values) * log_prob[samples]) - 0.01 *
                (prob * log_prob).sum())
        ...
        def loss_fn(
            ...
            (logits, spr_predictions, _) = get_logits(q_online, current_state,
                                                      actions[:, :-1], use_spr)
            logits = jnp.squeeze(logits)
            # Fetch the logits for its selected action. We use vmap to perform this
            # indexing across the batch.
            chosen_action_logits = jax.vmap(lambda x, y: x[y])(logits,
                                                               actions[:, 0])
            ...
            return mean_loss + jnp.mean(policy_loss(params, states, key)), (
                aux_losses)
        ...
        spr_targets = get_spr_targets(encode_project, future_states)
...
class BBFAgent(dqn_agent.JaxDQNAgent):
    ...
    def _build_replay_buffer(self):
        ...
        self._uniform_replay = subsequence_replay_buffer.OutOfGraphReplayBuffer(
            observation_shape=self.observation_shape,
            stack_size=self.stack_size,
            #replay_capacity=int(10e3),
            replay_capacity=int(100e3),
            batch_size=self._batch_size,
            observation_dtype=self.observation_dtype,
        )
