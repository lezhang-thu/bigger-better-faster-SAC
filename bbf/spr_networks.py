  ...
  """
    ...
    def setup(self):
        ...
        if self.r2_world_model_enabled:
            # The RSSM observes the dual-space projection concat
            # (project | policy_projection), 2 * hidden_dim wide, so its
            # bridge decodes latents directly into the two head-input
            # spaces (no separate adapter needed).
            self.r2_world_model = R2DreamerWorldModel(
                embed_size=2 * int(self.hidden_dim),
    ...
    def r2_q_pi_from_concat(self, concat, support):
        """Q values and policy logits from a (project|policy_projection)
        concat vector — either a real state's encode_project output or the
        bridge's prediction of one. Single sample (LinearHead reshapes)."""
        hidden = int(self.hidden_dim)
        q_logits = self.head(nn.relu(concat[..., :hidden]), eval_mode=True)
        probabilities = nn.softmax(q_logits)
        q_values = jnp.sum(support * probabilities, axis=-1)
        policy_logits = self.policy(nn.relu(concat[..., hidden:]))
        return q_values, policy_logits

    def r2_policy_logits_from_concat(self, concat):
        """Policy logits from the policy half of a projection concat.
        Batched (Dense handles leading dims)."""
        hidden = int(self.hidden_dim)
        return self.policy(nn.relu(concat[..., hidden:]))
    ...
    def init_fn(
        ...
        if self.r2_world_model_enabled:
            ...
            dummy_concat = jnp.zeros((2 * int(self.hidden_dim),),
                                     dtype=jnp.float32)
            _ = self.r2_q_pi_from_concat(dummy_concat, support)
        return (
            y,
    ...
    def r2_imagine(self, start_stoch, start_deter, horizon, unimix, rng):
        ...
        for i in range(horizon + 1):
            ...
            bridge_concat = jax.lax.stop_gradient(
                self.r2_world_model.bridge_predict(feat))
            logits = self.r2_policy_logits_from_concat(bridge_concat)
