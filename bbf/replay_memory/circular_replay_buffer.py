...
# debugging.
ReplayElement = (collections.namedtuple('shape_type',
                                        ['name', 'shape', 'type']))
...
class OutOfGraphReplayBuffer(object):
  ...
  """
    ...
    """
        self._check_args_length(*args)
        for arg_element, store_element in zip(args,
    ...
    """
        ...
        # it is invalid.
        for i in modulo_range(index, self._update_horizon,
            ...
            for element_array, element in zip(batch_arrays,
                ...
                if element.name == 'state':
                    ...
                    element_array[batch_element] = (self._store[
                        element.name.lstrip('next_')][(next_state_index) %
                                                      self._replay_capacity])
