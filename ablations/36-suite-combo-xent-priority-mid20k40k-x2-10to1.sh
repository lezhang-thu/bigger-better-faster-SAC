...
# 32a-suite-combo-xent-priority.sh, with exactly two training changes:
...
# discount retain 32a's coupled 0.97 -> 0.997 cycle schedule.
#
...
#
# Everything else is held to 32a: reward readouts, imagined actor/value losses,
# coupled imagination entropy/discount, cross-entropy PER with the host
# alpha=.5 square root, and ERE disabled. RUN=199 follows runs 194-198.
#
# Usage:
#   bash ablations/36-suite-combo-xent-priority-mid20k40k-x2-10to1.sh
#   GAMES="Kangaroo Asterix" GPU=1 RUN=199 \
#     bash ablations/36-suite-combo-xent-priority-mid20k40k-x2-10to1.sh
cd "$(dirname "$0")/.."
