set -ex
# One-step expected C51 plus an exact-policy-mixture, uncorrected ten-step
# lower-bound signal. The one-step C51 cross-entropy remains the primary critic
# objective.
...
# ten-step reward prefix and exact-policy-mixture S_10 bootstrap are
# reconstructed from the same sampled anchor. Gamma is fixed at 0.997.
#
# Usage:
...
for ((rep = 1; rep <= REPS; rep++)); do
	for game_name in $GAMES; do
		CUDA_VISIBLE_DEVICES=$GPU python -m bbf.train \
			...
			--gin_bindings="BBFAgent.td_lower_bound_weight=0.1" \
