...
## stage0123v2 (2026-07-12): two imagination engines on the corrected original

Built on `bbf-starting-point-claude` (true original architecture: no
representation_projection, dual-space SPR). Adds the RSSM stack as a second,
gin-selectable imagination engine beside the validated conv-TM engine:

- RSSM embedding = `sg(concat(project(x), policy_projection(x)))` (4096-d) —
  full encoder isolation; the bridge decodes latents straight into the two
  head-input spaces (`r2_q_pi_from_concat`), so imagination reads the shared
  policy/Q heads with no adapter.
- Imagined values: `V(z) = sum_a pi(a|z) Q_target(z,a)` on bridge halves —
  one critic in the whole agent (the mechanism behind the Pong 20.41 win).
  The twohot feat-value head remains for diagnostics only.
- Both engines share the reset-aware `imag_warmup` ramp.
- Configs: base `BBF-100K.gin` = conv engine on (validated defaults);
  `rssm-imag.gin` = RSSM engine on / conv off (matched H=5, actor 0.1);
  `anchor-only.gin` = pure baseline reproduction.
